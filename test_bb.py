...
from millybb import Millybb as BBAgent
...
def test_bb():
    ...
    verb_assert(a1.target_slot(t, history, reserve), (1, 5, 10))
    # bid for slot 1: 8 - 2/3 * (8 - 5) = 8 - 2/3(3) = 6
    dual_assert(a1.bid(t, history, reserve), 6)

    # a2's utils for slots: [3 * (10 - 10), 2*(10-4), 0] = [0, 12, 0]
    verb_assert(a2.expected_utils(t, history, reserve), [0, 12, 0])
    verb_assert(a2.target_slot(t, history, reserve), (1, 4, 10))
    # bid for slot 1: 10 - 2/3 * (10 - 4) = 10 - 4 = 6
    dual_assert (a2.bid(t, history, reserve) , 6)

    # a3's utils for slots: [3 * (20 - 5), 2*(20-4), 0] = [45, 32, 0]
    verb_assert(a3.expected_utils(t, history, reserve), [45, 32, 0])
    verb_assert(a3.target_slot(t, history, reserve), (0, 5, 10))
